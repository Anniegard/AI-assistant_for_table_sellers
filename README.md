...
- Запустить бота или Web API (`POST /api/demo/messages`).
- Пройти пошаговый сбор (Telegram: кнопки и текст; Web: один вопрос за раз + `quick_replies`):
  сценарий → рост → бюджет → число мониторов → системник на столе → размер/место.
- Можно сразу прислать пачку параметров свободным текстом — парсер подхватит числа
  с учётом текущего шага (рост vs бюджет) и фразы вроде «50–80к», «140×70».
- Получить 2–3 рекомендации; при нехватке вариантов в пределах бюджета допускается
  показ до **+15%** к верхней границе с пометкой в tradeoffs.
- После подбора: `Почему советуешь?`, `Подороже`, смена бюджета/сценария/размера,
  `Есть дешевле?`, `Оставь заявку`.
- Проверить JSON лидов и уведомление менеджеру при `MANAGER_TELEGRAM_CHAT_ID`.

## Пример диалога

**Web (пошагово):** сессия создаётся через `POST /api/demo/sessions`, далее сообщения в ту же `session_id`.

- Пользователь: `Привет`
- ЭргоАссистент: вопрос про сценарий (кнопки/чипы в `quick_replies`).
- Пользователь: `Для работы дома`
- ЭргоАссистент: запрос роста.
- Пользователь: `178`
- ЭргоАссистент: запрос бюджета (поддерживаются диапазоны «от 50 до 80к», «до 60 тысяч»).
- … после мониторов, ПК и размера — рекомендации с русскими подписями сценария (без `home_office` в тексте для пользователя).

**Один длинный ответ (и бот, и API):**

- Пользователь: `Стол для дома, рост 185, два монитора, системник на столе, до 70к, столешница максимум 140×70`
- ЭргоАссистент: подбор с учётом сценария, эргономики по росту, ширины и нагрузки; при необходимости — варианты с лёгким превышением бюджета до +15%.

- Пользователь: `Почему не один мотор?`
- ЭргоАссистент: пояснение по каталогу (два мотора / запас стабильности при двух мониторах и т.п.).
- Пользователь: `Оставь заявку` → сбор контактов.
...
- Свободный текст поддерживает извлечение `сценарий`, `рост`, `бюджет (min/max)`, `мониторы`, `ПК на столе`, `ограничение по размеру` и накопление параметров между сообщениями.
- В guided flow ассистент идет по шагам: сценарий → рост → бюджет → мониторы → ПК на столе → размер/место; уже известные параметры не переспрашиваются.
- Для бюджета поддерживаются форматы `50-80к`, `до 60к`, `примерно 70 тысяч`; при нехватке строгих вариантов возможен показ до `+15%` к `budget_max` с явной пометкой компромисса.
